        ...
        <b-card-body class="p-4">
          <b-card no-body class="mb-4">
            <b-row class="align-items-center">
              ...
              <b-col sm="6" md="3" >
                <b-button v-if="isCancelled" :disabled="isLoading" variant="primary" class="mt-2 btn-danger" @click="showAlert"> Cancel
                  Now
                ...
                <b-button variant="primary" class="mt-2 ml-2 btn-primary" @click="redirectUser">
                  <BIconArrowLeft/> Back
                </b-button>
              </b-col>
            </b-row>
...
import { BookingDetails, HotelDetailStore, useAuthStore } from '@/stores/auth'
...
bookingDetails = bookingDetails.getUserData();
roomInformation.value = bookingDetails;
console.log(roomInformation.value.status);

if(roomInformation.value.status == "completed" && moment(roomInformation.value.checkIn).format("YYYY-MM-DD") < moment().format("YYYY-MM-DD")){
  isCancelled.value = false
  console.log("Value is flase now");
  
}
console.log(roomInformation.value);
console.log(roomInformation.value.specialRequest);
hotelRating.value = roomInformation.value.extraDetails.hotelDetails.rating
hotelTitle.value = roomInformation.value.extraDetails.hotelDetails.title
HotelAddress.value = roomInformation.value.extraDetails.hotelDetails.address
HotelImage.value = roomInformation.value.extraDetails.hotelDetails.image
roomDetails.value = roomInformation.value.extraDetails.roomsDetail;
specailRequest.value = roomInformation.value.specialRequest;
Guest.value = roomInformation.value.extraDetails.guestDeatils.length;
mainGuest.value = roomInformation.value.extraDetails.mainGuest;
allGuest.value = roomInformation.value.extraDetails.guestDeatils
let daysNight = calculateDateDifference(roomInformation.value.checkIn, roomInformation.value.checkOut);
