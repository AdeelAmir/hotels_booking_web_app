<template>
  <b-card-header class="border-bottom d-md-flex justify-content-md-between align-items-center">
    <div class="d-flex align-items-center">
      <div v-if="booking.hotelImage == ''" class="icon-lg bg-light rounded-circle flex-shrink-0">
        <font-awesome-icon :icon="faHotel" />
      </div>
      <div v-else class="avatar me-1 mt-2">
        <img class="avatar-img rounded-circle shadow" :src="booking.hotelImage" alt="avatar" />
      </div>
      <div class="ms-2">
        <b-card-title tag="h6" class="mb-0">{{ booking.hotelName }}</b-card-title>
