...
const HotelDetail = HotelDetails.getUserData();
let iVectorData = [];
if (HotelDetail?.iVectorData?.PropertyResults && HotelDetail.iVectorData.PropertyResults.length > 0) {
  iVectorData = HotelDetail.iVectorData.PropertyResults[0].RoomTypes || [];
}

console.log(iVectorData);
...
function onRoomSelect(mealOption: any, room:any) {
  mealOption.bookingToken = HotelDetail.iVectorData.PropertyResults[0].BookingToken;
  mealOption.propertyId = HotelDetail.iVectorData.PropertyResults[0].PropertyID;
  mealOption.roomType = room.room_name;
  // Set numberOfRooms to 1 if selected and initially 0
