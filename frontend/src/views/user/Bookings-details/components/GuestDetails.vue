      ...
      <b-form class="row g-4">
        ...
        <b-col md="6">
          <b-form-group label="Email id" label-for="email">
            ...
            <b-form-text>(Booking voucher will be sent to this email ID)</b-form-text>
          </b-form-group>
        </b-col>
...
const userDetails = useAuth.getUserData();
...
function logFormData() {
  // let specialRequests:any = specialRequestValues.value;
  // specialRequests = specialRequests.filter((item:any) => item.id == specialRequests.id && item.id == true)
  const formData = {
    name: name.value,
    email: email.value,
